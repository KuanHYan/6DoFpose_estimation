...
train_pipeline = [
    ...
    dict(type='LoadPointCloud'),
    dict(type='AngleClassificationLabel', n_theta=180, n_phi=360),
    dict(type='CustomResize', scale=(480, 480), keep_ratio=True),
    # dict(type='RandomFlip', prob=0.5),
    dict(
        type='PackDetInputs',
        meta_keys=(
            'img_id', 'img_path', 'ori_shape', 'img_shape', 'intrinsic_matrix',
            'target_img_path', 'scale_factor', 'flip', 'flip_direction',
            'rotation', 'translation', 'center', 'point_cloud', 'angle_labels')
        )
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    ...
    dict(
        ...
        meta_keys=(
            ...
            'target_img_path', 'rotation', 'translation', 'intrinsic_matrix',
            'center', 'point_cloud', 'angle_labels', 'scale_factor')
        )
]
# # TODO: 自定义数据集Pipeline
# train_pipeline = [
...
model = dict(
    ...
    roi_head=dict(
        type='PoseHead',
        rot_use_add_loss=True,
        tran_use_center=True,
        roi_extractor=dict(
    ...
    train_cfg=dict(
        ...
        rcnn=dict(
            assigner=dict(
                ...
                ignore_iof_thr=-1),
            sampler=dict(num=32, pos_fraction=0.5),
        )
    ),
)
...
# 数据集设置
# ----------------------------
data_root = user_root + '/dataset/NOCS'
...
optim_wrapper = dict(
    optimizer=dict(lr=0.002, weight_decay=4.0e-5),
    paramwise_cfg=dict(
        custom_keys={
            'backbone.layer4': dict(lr_mult=0.02)  # 降低骨干网络的学习率（如果未完全冻结）
        }
    )
)
param_scheduler = [
    # NOTE: start_lr = start_factor * base_lr, i.e., 0.01 * 0.32 = 0.0032
    dict(type='LinearLR', start_factor=0.001, by_epoch=False, begin=0, end=2048),
    dict(type='MultiStepLR', by_epoch=True, begin=0, milestones=[70, 80, 90], gamma=0.5)
]
...
custom_hooks = [
    ...
    dict(type='UpdateJointAngleClassificationHeadTemp', module_name='roi_head.rotation_head')
]
